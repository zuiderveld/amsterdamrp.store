...
export async function loadSettingsAsync(dataDir, seedDir, fallback) {
  ...
  if (remote && typeof remote === "object") {
    // Overlay lokale/payment snapshot zodat Stripe-toggle niet terugspringt
    for (const dir of [dataDir, seedDir]) {
      try {
        const snap = JSON.parse(fs.readFileSync(path.join(dir, "payments-settings.json"), "utf8"));
        if (snap?.payments) {
          remote.payments = {
            stripe: Boolean(snap.payments.stripe),
            tebex: Boolean(snap.payments.tebex),
          };
          break;
        }
      } catch {
  ...
  // Payments snapshot wint altijd voor toggles (voorkomt reset na refresh)
  for (const dir of [dataDir, seedDir]) {
    try {
      const snap = JSON.parse(fs.readFileSync(path.join(dir, "payments-settings.json"), "utf8"));
      if (snap?.payments) {
